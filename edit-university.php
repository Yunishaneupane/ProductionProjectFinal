...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $country && $imageUrl && $description) {
        $updateQuery = "UPDATE universities SET name = ?, country = ?, image_url = ?, ranking = ?, description = ? WHERE university_id = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("sssisi", $name, $country, $imageUrl, $ranking, $description, $universityId);
...
<div class="form-container">
    ...
    <form method="POST" class="add-form">
        ...
        <div id="programs-container">
            ...
            <!-- Blank block to add more -->
            <div class="program-block">
                <input type="hidden" name="program_ids[]" value="">
                <input type="text" name="programs[]" placeholder="Program Name">
                <input type="text" name="levels[]" placeholder="Level (e.g., Bachelor's)">
                <input type="number" name="durations[]" placeholder="Duration (years)" min="1">
                <input type="number" step="0.01" name="fees[]" placeholder="Tuition Fee">
            </div>
        </div>
